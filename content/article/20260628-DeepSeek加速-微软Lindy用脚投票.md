...
DeepSeek 的定价本来就低——V4 Pro 的 output 端每百万 token 只要 $0.87（今年 5 月 22 日永久降价 75% 之后的数字）。DSpark 让每一块钱能买到的 token 又多了 50%+。一个本来已经在性价比上碾压所有人的选手，又踩了一脚油门。
...
但微软不是"转投"DeepSeek。它在同一个产品里同时铺了 OpenAI + Anthropic + 自研 + DeepSeek 四条线。这不是站队，是多模型路由——模型从信仰变成了零件。

这里有一个容易被忽略的时间线细节。2026 年 4 月 27 日，微软和 OpenAI 彻底重写了合作协议：OpenAI 可以向任何云厂商销售（不再 Azure 独占），微软不再向 OpenAI 支付 revenue share，AGI 条款正式删除。两个月后，微软 AI 部门 CEO Mustafa Suleyman 公开说约六个月前获得合同许可后，正式开始独立追求超级智能，微软也拿出了自己的 MAI-Thinking-1 等模型。正是在双方关系结构性裂变的背景下，Copilot Cowork 开始评估 DeepSeek。

这不是突然的"背叛"，而是半年以来的连锁反应——合作关系松动、自有模型起步、按量计费倒逼成本控制、开源模型成熟——四件事碰到了一起。
...
有意思的是，同一时间它也开源了 DFlash 和 Eagle3 两种基线推测解码方法的训练代码。DeepSeek 正在变成推测解码（speculative decoding）领域的标准基准框架。
...
还有一个信号：新加坡为其国家 AI 模型选择了基于阿里 Qwen 而非 Meta Llama 来构建。这是"主权 AI"话语下，中国开源模型被选作默认基座的第一个政府级案例。

...
| ⑤ 审稿 | `/review-draft` | 逐条检查 StyleRule（6 类高频违规） | 审核意见（对话中） |
...
| `/review-draft` | 逐条检查 StyleRule（6 类高频违规）+ 数据核对 + 逻辑审核 | 已完成 `/create-draft`，`content/draft/` 有草稿 | 审核意见（对话中） |
...
| **`StyleRule.md`** | 编辑风格规则（6 条）：作者隐身、弱化绝对、论据收敛、叙事纯净、列表化 |

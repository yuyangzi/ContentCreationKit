...
4. 以 `{YYYYMMDD}-{主题名}.md` 的文件名保存到 `content/WeChat/` 目录。
...
- `content/WeChat/{YYYYMMDD}-{主题名}.md`：公众号排版后的文章文件
- `content/WeChat/{YYYYMMDD}-{主题名}/article.html`：微信兼容的 HTML 文件（可直接复制到公众号后台）
- `content/WeChat/{YYYYMMDD}-{主题名}/preview.html`：手机预览页面

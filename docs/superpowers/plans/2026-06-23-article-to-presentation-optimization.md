...
- [ ] **Step 2: 确认新增组件在 v1.0.8 中存在**

Run:
```bash
# 利用已有安装
ls content/ppt/2026-06-22-AI-execution-judgment/node_modules/@enyineer/slidev-theme-neocarbon/components/ 2>/dev/null | grep -iE "terminal|steps|flip|roi|heatmap"
```

若目录不存在 → 先安装：
```bash
cd content/ppt/2026-06-22-AI-execution-judgment && npm install --registry https://registry.npmmirror.com
```
然后重试 ls。
...
若某组件不存在 → Task 2 中为该组件文档添加 `⚠️ 需验证` 标注（spec §7.2）。

- [ ] **Step 3: 验证 `clickAnimation` frontmatter 字段**

Run:
```bash
cd content/ppt/2026-06-22-AI-execution-judgment && grep -r "clickAnimation" node_modules/@enyineer/slidev-theme-neocarbon/ node_modules/@slidev/cli/ 2>/dev/null | head -20
```

若 grep 无结果 → `clickAnimation` 不存在于 neocarbon 或 Slidev 源码中。改为 CSS-only 方案：
- 不写 `clickAnimation` frontmatter
- 动画降级仅靠 `<style>` CSS 控制
- spec §2.2 / SKILL.md §2.2 / technical-details §3.3 中的 frontmatter 行改为注释说明
...
Run:
```bash
grep -c "布局\|layout\|NcBar\|NcLine\|NcTerminal\|NcSteps\|NcFlip\|NcRoi\|browser\|spotlight\|diagram\|Mermaid" .opencode/skills/article-to-presentation/SKILL.md
echo "---"
grep -c "布局\|layout\|NcBar\|NcLine\|NcTerminal\|NcSteps\|NcFlip\|NcRoi\|browser\|spotlight\|diagram\|Mermaid" .opencode/skills/article-to-presentation/references/technical-details.md
```
Expected: SKILL.md 映射表区域 16 行（8 + 8 新增），technical-details.md 映射表区域 19 行（13 + 6 新增）。SKILL.md 是 technical-details 的精炼子集。

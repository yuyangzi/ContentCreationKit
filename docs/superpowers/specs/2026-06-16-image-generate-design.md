...
# === 参数解析 ===
PROMPT = sys.argv[1]       # 必填：生图提示词
NAME = sys.argv[2]          # 必填：文件名（不含扩展名）
...
| API 调用失败 | `openai.APIError` 异常 | 显示错误信息，退出 |
| 图片下载失败 | `urllib` 异常 | 显示 URL 和错误，退出 |
| prompt 为空 | 参数检查 | 提示用法，退出 |
...
| # | 场景 | 输入 | 预期结果 |
|---|------|------|----------|
| 1 | 正常生成 | prompt="一只猫", name="test-cat" | `content/images/test-cat.png` 存在 |
| 2 | API Key 缺失 | ARK_API_KEY 未设置 | 提示错误，退出 |
...
# 测试生成
python3 -c "
import os; os.environ['ARK_API_KEY']='test-key'
# ... 实际测试需要真实 API Key
"

# 验证文件
ls -la content/images/test-cat.png
```
